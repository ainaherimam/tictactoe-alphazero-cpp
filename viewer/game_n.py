#!/usr/bin/env python3
"""
All-in-one server for NORMAL 4×4 Neural Tic-Tac-Toe.
...
<title>NORMAL 4×4 — Neural Tic-Tac-Toe</title>
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header h1 {
    ...
    font-size: 3rem;
    letter-spacing: 3px;
    background: linear-gradient(135deg, var(--accent-o), var(--accent-x));
  ...
  .cell.winning { animation: win-pulse 0.6s ease-in-out 2; }
  @keyframes win-pulse {
    0%,100% { box-shadow: none; }
    50% { box-shadow: 0 0 20px var(--accent-o), inset 0 0 10px rgba(0,201,167,0.2); }
  }
  ...
  .game-result .winner-text.X { color: var(--accent-x); }
  .game-result .winner-text.O { color: var(--accent-o); }
  ...
  .btn-undo { color: #a0a0c0; border-color: rgba(160,160,192,0.4); }
  .btn-undo:hover { background: rgba(160,160,192,0.08); }
  .btn-undo:disabled { opacity: 0.3; cursor: not-allowed; }
  .btn-validate { border-color: var(--accent-o); color: var(--accent-o); background: rgba(0,201,167,0.15); flex: 1; animation: pulse-btn 1s ease-in-out infinite; }
  .btn-validate:hover { background: rgba(0,201,167,0.25); box-shadow: 0 0 16px var(--glow-o); }
...
<div class="container">
  ...
  <header>
    <h1>NORMAL 4×4</h1>
    <div class="subtitle">Neural Tic-Tac-Toe<br>3-in-a-row WINS</div>
  </header>
    ...
    <div class="main-layout">
      ...
      <div class="board-wrap">
        <div class="turn-bar">
          ...
          <div class="rule-badge">3 in a row = WIN</div>
        </div>
...
let moveHistory = [];
let aiShowing = false;
let aiPendingIdx = -1;  // index of AI's chosen move, waiting for validation
...
function handleCellClick(idx) {
  ...
  // If AI is in validate mode, a manual click cancels it and plays the clicked cell
  if (aiPendingIdx >= 0) {
    cancelAIPending();
  }
...
function endGame(result) {
  ...
  if (!result) {
    ...
    winnerSub.textContent = `${winner} formed 3-in-a-row!`;
    line.forEach(i => document.getElementById(`cell-${i}`).classList.add('winning'));
  }
  const tp = document.getElementById('turnPlayer');
  tp.textContent = result ? result.winner + ' WIN' : 'DRAW';
  const btnAI = document.getElementById('btnAI');
  btnAI.disabled = true; btnAI.className = 'btn btn-ai'; btnAI.textContent = '⚡ Ask AI';
  aiPendingIdx = -1;
}
...
function resetGame() {
  ...
  currentPlayer = 'X'; gameOver = false; moveHistory = []; aiShowing = false; aiPendingIdx = -1;
  ...
  const btnAI = document.getElementById('btnAI');
  btnAI.disabled = false; btnAI.className = 'btn btn-ai'; btnAI.textContent = '⚡ Ask AI';
...
function handleAIButton() {
  if (gameOver) return;
  if (aiPendingIdx >= 0) {
    // Validate: apply the AI's chosen move
    const idx = aiPendingIdx;
    cancelAIPending(false);
    placeMove(idx, currentPlayer);
  } else {
    requestAI();
  }
...
function cancelAIPending(clearOverlay = true) {
  aiPendingIdx = -1;
  const btn = document.getElementById('btnAI');
  btn.disabled = gameOver;
  btn.className = 'btn btn-ai';
  btn.textContent = '⚡ Ask AI';
  if (clearOverlay) clearAIOverlay();
}
...
async function requestAI() {
  ...
  try {
    ...
    // Highlight AI's best move
    if (bestIdx >= 0) {
      const bestCell = document.getElementById(`cell-${bestIdx}`);
      bestCell.style.outline = '2px solid var(--accent-o)';
      bestCell.style.outlineOffset = '-2px';
    }
    aiPendingIdx = bestIdx;
    // Switch button to Validate mode
    btn.disabled = false;
    btn.classList.remove('loading');
    btn.className = 'btn btn-validate';
    btn.textContent = '✓ Validate AI Move';
  } catch(err) {
    setStatus('err', err.message.slice(0,60)); console.error(err);
    btn.disabled = gameOver; btn.classList.remove('loading'); btn.textContent='⚡ Ask AI';
  }
}

function undoMove() {
  if (moveHistory.length === 0 || gameOver) return;
  // If AI is pending validation, cancel it first
  if (aiPendingIdx >= 0) cancelAIPending(true);
  const last = moveHistory.pop();
  board[last.idx] = EMPTY;
  currentPlayer = last.player; // restore turn to the player who made that move
  clearAIOverlay();
  // Rebuild history display
  const list = document.getElementById('historyList');
  list.innerHTML = '';
  moveHistory.forEach(h => addHistory(h.player, h.idx));
  document.getElementById('btnUndo').disabled = moveHistory.length === 0;
  // Hide value panel if no AI showing
  if (!aiShowing) document.getElementById('valuePanel').classList.remove('visible');
  renderBoard();
}
...
function showAIOverlay(policy, value) {
  ...
  const vLabel=document.querySelector('#valuePanel .value-label');
  ...
  if (vLabel) vLabel.textContent = '🧠 Neural Value — Current Player';
...
if __name__ == "__main__":
    ...
    print(f"╔══════════════════════════════════════════════╗")
    print(f"║  NORMAL 4×4 — Neural Tic-Tac-Toe            ║")
    print(f"╠══════════════════════════════════════════════╣")
