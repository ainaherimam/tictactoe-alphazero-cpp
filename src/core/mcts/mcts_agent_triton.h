#ifndef MCTS_AGENT_TRITON_H
#define MCTS_AGENT_TRITON_H
...
/**
 * @brief Implements a Monte Carlo Tree Search (MCTS) agent for decision-making in games using Triton inference
 *
...
class Mcts_agent_triton {
public:
    /**
     * @brief Constructs a new MCTS agent using Triton inference
     *
     * @param config Configuration struct containing all MCTS parameters
    */
    Mcts_agent_triton(const Mcts_triton_config& config);
...
private:
    ...
    */
    std::pair<std::shared_ptr<Mcts_agent_triton::Node>, std::vector<float>> sample_child_and_get_policy(
     ...
     */
    std::pair<std::shared_ptr<Mcts_agent_triton::Node>, Board> select_child_for_playout(
