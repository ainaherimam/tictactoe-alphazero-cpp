...
void selfplay_worker(int worker_id,
    ...
    for (int game_num = 0; game_num < games_per_worker; ++game_num) {
        ...
        Mcts_triton_config config(1.4, 100, LogLevel::NONE, 0.0, 0.3, 0.25, client, -1, false, 0);
        
        auto player1 = std::make_unique<Mcts_player_triton>(config);
        auto player2 = std::make_unique<Mcts_player_triton>(config);
...
int main(int argc, char** argv) {
    ...
    std::string model_name = "mcts_model";      // Model name in Triton
    ...
    for (int i = 0; i < NUM_WORKERS; ++i) {
        workers.emplace_back(selfplay_worker, i, shm_writer, &client, game_logger, GAMES_PER_WORKER);
    }
