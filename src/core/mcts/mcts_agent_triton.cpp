...
Mcts_agent_triton::Mcts_agent_triton(const Mcts_triton_config& config)
    : exploration_factor(config.exploration_factor),
      number_iteration(config.number_iteration),
      logger(Logger::instance(config.log_level)),
...
std::vector<float> Mcts_agent_triton::generate_dirichlet_noise(int num_moves, float alpha) {
...
std::pair<Move, std::vector<float>> Mcts_agent_triton::choose_move(const Board& board, Cell_state player) {
...
void Mcts_agent_triton::perform_mcts_iterations(int number_iteration, int& mcts_iteration_counter, const Board& board) {
...
std::pair<std::shared_ptr<Mcts_agent_triton::Node>, std::vector<float>> Mcts_agent_triton::sample_child_and_get_policy(
...
std::pair<std::shared_ptr<Mcts_agent_triton::Node>, Board> Mcts_agent_triton::select_child_for_playout(
...
double Mcts_agent_triton::calculate_puct_score(const std::shared_ptr<Node>& child_node,
...
void Mcts_agent_triton::backpropagate(std::shared_ptr<Node>& node, float value) {
