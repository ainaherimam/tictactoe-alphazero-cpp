...
void eval_worker_best_as_p1(int worker_id,
    ...
    for (int game_num = 0; game_num < games_per_worker; ++game_num) {
        ...
        auto player1 = std::make_unique<Mcts_player_selfplay>(
            1.4, 100, LogLevel::NONE, 0.0, 0.3, 0.0, queue_best, -1, false, 0);
        auto player2 = std::make_unique<Mcts_player_selfplay>(
            1.4, 100, LogLevel::NONE, 0.0, 0.3, 0.0, queue_candidate, -1, false, 0);
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        std::cout << "Initializing inference queues...\n";
        
        // Load the two inference queues
        ...
        if (!queue_best->wait_for_server(30000)) {
            std::cerr << "Error: Best model inference server not ready!\n";
            std::cerr << "Skipping this checkpoint and waiting for next trigger...\n";
            continue;
        }
        ...
        if (!queue_candidate->wait_for_server(30000)) {
            std::cerr << "Error: Candidate model inference server not ready!\n";
            std::cerr << "Skipping this checkpoint and waiting for next trigger...\n";
            continue;
        }
        ...
        for (int i = 0; i < NUM_WORKERS; ++i) {
            workers.emplace_back(eval_worker_best_as_p1, i, queue_best, queue_candidate, 
                               results, GAMES_PER_WORKER_PER_SIDE);
        }
        
        // Spawn workers for best model as Player 2
        for (int i = 0; i < NUM_WORKERS; ++i) {
            workers.emplace_back(eval_worker_best_as_p2, i + NUM_WORKERS, queue_best, 
                               queue_candidate, results, GAMES_PER_WORKER_PER_SIDE);
        }
